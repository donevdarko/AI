...
dataset_modified = [[val for i, val in enumerate(row) if i != col_index] for row in dataset]

split = len(dataset_modified)*85//100

train_X = [row[:-1] for row in dataset_modified[:split]]
train_Y = [row[-1] for row in dataset_modified[:split]]
test_X = [row[:-1] for row in dataset_modified[split:]]
test_Y = [row[-1] for row in dataset_modified[split:]]

classifier = RandomForestClassifier(n_estimators=num_trees, random_state=0, criterion=criteria)
...
record_modified = [val for i, val in enumerate(record) if i!=col_index]
pred = classifier.predict([record_modified])
probas = classifier.predict_proba([record_modified])
