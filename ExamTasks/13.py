...
if __name__ == '__main__':
    ...
    dataset_0_class = [row for row in dataset if row[-1] == 0]
    dataset_1_class = [row for row in dataset if row[-1] == 1]
    dataset_2_class = [row for row in dataset if row[-1] == 2]

    split_0 = int(len(dataset_0_class)*x1)
    split_1 = int(len(dataset_1_class)*x1)
    split_2 = int(len(dataset_2_class)*x1)

    split_test_0 = int(len(dataset_0_class)*x2)
    split_test_1 = int(len(dataset_1_class)*x2)
    split_test_2 = int(len(dataset_2_class)*x2)

    test_data = dataset_0_class[split_test_0:] + dataset_1_class[split_test_1:] + dataset_2_class[split_test_2:]
    test_X = [row[:-1] for row in test_data]
    test_Y = [row[-1] for row in test_data]

    #Naive Bayes
    train_data = dataset_0_class[:split_0] + dataset_1_class[:split_1] + dataset_2_class[:split_2]

    train_X_bayes = [row[:-1] for row in train_data]
    train_Y_bayes = [row[-1] for row in train_data]

    naive_bayes_classifier = GaussianNB()
    naive_bayes_classifier.fit(train_X_bayes, train_Y_bayes)

    ###

    #Decision Tree

    train_data_tree =  dataset_0_class[split_0:split_test_0] + dataset_1_class[split_1:split_test_1] + dataset_2_class[split_2:split_test_2]

    train_X_tree = [row[:-1] for row in train_data_tree]
    train_Y_tree = [row[-1] for row in train_data_tree]

    decision_tree_classifier = DecisionTreeClassifier(random_state=0)
    decision_tree_classifier.fit(train_X_tree, train_Y_tree)

    ###

    #Random Forest

    train_data_forest = dataset_0_class[:split_test_0] + dataset_1_class[:split_test_1] + dataset_2_class[:split_test_2]

    train_X_forest = [row[:-1] for row in train_data_forest]
    train_Y_forest = [row[-1] for row in train_data_forest]

    forest_classifier = RandomForestClassifier(n_estimators=3, random_state=0)
    forest_classifier.fit(train_X_forest, train_Y_forest)

    ###


    naive_pred = naive_bayes_classifier.predict(test_X)
    tree_pred = decision_tree_classifier.predict(test_X)
    forest_pred = forest_classifier.predict(test_X)

    correct=0

    for naive, tree, forest, y in zip(naive_pred, tree_pred, forest_pred, test_Y):
        curr=0
        if naive == y:
            curr+=1
        if forest == y:
