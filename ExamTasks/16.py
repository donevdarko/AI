...
if __name__ == '__main__':
    dataset_0_class = [row for row in dataset if row[-1] == "Perch"]
    dataset_1_class = [row for row in dataset if row[-1] == "Roach"]
    dataset_2_class = [row for row in dataset if row[-1] == "Bream"]

    join_dataset = [dataset_0_class, dataset_1_class, dataset_2_class]

    p = int(input())
    drakula = input()
    l = int(input())

    split = len(dataset)*p//100
    
    dataset_train = dataset[:split]
    dataset_test = dataset[split:]

    train_X = [row[:-1] for row in dataset_train]
    train_Y = [row[-1] for row in dataset_train]
    test_X = [row[:-1] for row in dataset_test]
    test_Y = [row[-1] for row in dataset_test]

    tree_clasifier = DecisionTreeClassifier(criterion=drakula, max_leaf_nodes=l, random_state=0)
    tree_clasifier.fit(train_X, train_Y)
    ...
    ### 3 trees
    for cls in classes:
        binary_Y = [1 if cls == label else 0 for label in train_Y]
        tree = DecisionTreeClassifier(criterion=drakula, max_leaf_nodes=l, random_state=0)
        tree.fit(train_X, binary_Y)
        drva[cls] = tree

    acc1 = tree_clasifier.score(test_X, test_Y)
    print(f"Tochnost so originalniot klasifikator: {acc1}")

    hits = 0

    for check, truth in zip(test_X, test_Y):
        predictions = {}
        for cls, tree in drva.items():
            predictions[cls] = tree.predict([check])[0]

        if predictions[truth] == 1 and sum(predictions.values()) == 1:
            hits+=1
    
    acc2 = hits/len(test_X)
