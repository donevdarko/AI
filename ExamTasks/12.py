...
if __name__ == '__main__':
    ...
    validate_X = [row[:-1] for row in validate]
    validate_Y = [row[-1] for row in validate]
    ...
    classifier = MLPClassifier(hidden_layer_sizes=neuron_num, activation='relu', learning_rate_init=learing_rate, max_iter=20)
    classifier.fit(train_X, train_Y)


    pred_1 = classifier.predict(train_X)
    hits = 0
    for p, truth in zip(pred_1, train_Y):
        if p == truth:
            hits+=1
    train_acc = hits/len(train_Y)

    pred_2 = classifier.predict(validate_X)
    hits = 0
    for p, truth in zip(pred_2, validate_Y):
        if p == truth:
            hits+=1
    val_acc = hits/len(validate_Y)
    ...
    if train_acc > val_acc * 1.15:
        ...
        record = [record]
        record = [row[:col_index] + row[col_index+1:] for row in record]
        ...
        pred = classifier.predict(record)[0]
