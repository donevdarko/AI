...
if __name__ == '__main__':
    ...
    split_procent = int(input())

    

    data_0 = [row for row in dataset if row[-1] == 0]
    data_1 = [row for row in dataset if row[-1] == 1]

    split = len(data_0)*split_procent//100

    train = data_0[:split]

    split = len(data_1)*split_procent//100
    test = data_0[split:]

    train_X_0 = [row[:-1] for row in train]
    train_Y_0 = [row[-1] for row in train]
    test_X_0 = [row[:-1] for row in test]
    test_Y_0 = [row[-1] for row in test]

    train = data_1[:split]
    test = data_1[split:]

    train_X_1 = [row[:-1] for row in train]
    train_Y_1 = [row[-1] for row in train]
    test_X_1 = [row[:-1] for row in test]
    test_Y_1 = [row[-1] for row in test]

    train_X = train_X_0 + train_X_1
    train_Y = train_Y_0 + train_Y_1
    test_X = test_X_0 + test_X_1
    test_Y = test_Y_0 + test_Y_1

    classifier_naive = GaussianNB()
    classifier_naive.fit(train_X, train_Y)
    classifier_tree = DecisionTreeClassifier(criterion='entropy', random_state=0)
    classifier_tree.fit(train_X, train_Y)
    classifier_forest = RandomForestClassifier(n_estimators=4, criterion='entropy', random_state=0)
    classifier_forest.fit(train_X, train_Y)
    classifier_net = MLPClassifier(hidden_layer_sizes=(10,), activation='relu', learning_rate_init=0.001, random_state=0)
    classifier_net.fit(train_X, train_Y)




    models = [classifier_naive, classifier_tree, classifier_forest, classifier_net]
    accs = []
    for model in models:
    ...
    for question in test_X:
        ...
        custom_preds.append(predicted_class)
    
    TP, FN = 0, 0
    for p, truth in zip(custom_preds, test_Y):
        if p == 1 and p == truth:
            TP+=1
        elif p == 0 and p != truth:
            FN+=1
