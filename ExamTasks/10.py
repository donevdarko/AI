...
if __name__ == '__main__':
    ...
    dataset = [row[:-1] + [1 if row[-1] >= 5 else 0] for row in dataset]
    
    x = int(input())

    split = len(dataset) * x // 100
    ...
    feat_imp = classifier.feature_importances_.tolist()

    worst_index = feat_imp.index(min(feat_imp))

    modified_dataset = [row[:worst_index] + row[worst_index+1:] for row in dataset]
    ...
    min_max_train_X = min_max_scaler.fit_transform(train_X)
    min_max_test_X = min_max_scaler.transform(test_X)

    standard_scaler = StandardScaler()
    standard_train_X = standard_scaler.fit_transform(train_X)
    standard_test_X = standard_scaler.transform(test_X)

    neuron_classifier = MLPClassifier(hidden_layer_sizes=(15,), activation='relu', learning_rate_init=0.001, max_iter=200, random_state=0)

    neuron_classifier.fit(standard_train_X, train_Y)
    acc1 = neuron_classifier.score(standard_test_X, test_Y)

    neuron_classifier.fit(min_max_train_X, train_Y)
    acc2 = neuron_classifier.score(min_max_test_X, test_Y)
