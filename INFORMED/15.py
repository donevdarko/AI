...
class Zadaca(Problem):
    ...
    def successor(self, state):
        ...
        movements = self.move[orientation]
        changeOrientation = self.turn[orientation]

        for action, (dx, dy) in movements.items():
            nx, ny = x + dx, y + dy
            if 0<=nx<10 and 0<=ny<10 and (nx, ny) not in snakebody:
    ...
    def h(self, node):
        snake, orientation, green = node.state
        return len(green)
...
if __name__ == "__main__":
    ...
    problem = Zadaca((snake, direction, zeleni_jabolki))
