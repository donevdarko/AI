...
class Zadaca(Problem):
    ...
    def successor(self, retep):
        ...
        for i, pillar in enumerate(state):
            if len(pillar) == 0:
                continue
            disk = pillar[-1]
            for j, next_pillar in enumerate(state):
                if i == j:
                    continue
                if len(next_pillar) == 0:
                    new_next_pillar = list(next_pillar)
                    new_next_pillar.append(disk)
                    new_next_pillar = tuple(new_next_pillar)
                    new_pillar = list(pillar)
                    new_pillar.pop()
                    new_pillar = tuple(new_pillar)
                    new_state = list(state)
                    new_state[i] = new_pillar
                    new_state[j] = new_next_pillar
                    new_state = tuple(new_state)
                    successors[f'MOVE TOP BLOCK FROM PILLAR {i+1} TO PILLAR {j+1}'] = (new_state, actions+1)
                if len(next_pillar) > 0 and next_pillar[-1] >= disk:
                    new_next_pillar = list(next_pillar)
                    new_next_pillar.append(disk)
                    new_next_pillar = tuple(new_next_pillar)
                    new_pillar = list(pillar)
    ...
    def goal_test(self, state):
        if state[0] == self.final:
            print(f"Number of action {state[1]}")
            return True
        return False
...
if __name__ == "__main__":
    ...
    for torka in vlezniTorki:
        if torka:
            goal.append(tuple(map(int, torka.split(","))))
        else:
            goal.append(())
    goal = tuple(goal)

    problem = Zadaca((pillars, 0), goal)
