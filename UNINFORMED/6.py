...
class Zadaca(Problem):
    ...
    def successor(self, state):
        ...
        snake, orientation, green = state

        snakehead, snakebody = snake
        x, y = snakehead

        directions = self.slide[orientation]
        changeOrientation = self.turn[orientation]

        for action, (dx, dy) in directions.items():
            nx, ny = x + dx, y + dy
            if 0<=nx<10 and 0<=ny<10 and (nx, ny) not in red and (nx, ny) not in snakebody:
                new_snakebody = snakebody
                new_orientation = orientation
                new_green = green
                if action in ("Turn left", "Turn right"):
                    new_orientation = changeOrientation[action]
                
                
                if (nx, ny) in green:
                    new_green = tuple(t for t in green if t != (nx, ny))
                    new_snakebody = list(snakebody)
                    new_snakebody.append(snakehead)
                    new_snakebody = tuple(new_snakebody)
                
                if (nx, ny) not in green:
                    new_snakebody = snakebody[1:]
                    new_snakebody = list(new_snakebody)
                    new_snakebody.append(snakehead)
                    new_snakebody = tuple(new_snakebody)
                    
                successors[action] = (((nx, ny), new_snakebody), new_orientation, new_green)
...
if __name__ == '__main__':
    x = int(input())
    green = tuple([tuple(map(int, input().split(","))) for _ in range(x)])
    y = int(input())
    red = tuple([tuple(map(int, input().split(","))) for _ in range(y)])
    ...
    problem = Zadaca((snake, direction, green), red, None)
