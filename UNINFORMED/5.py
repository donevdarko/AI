...
class Zadaca(Problem):
    ...
    def successor(self, state):
        ...
        position, orientation, dots = state
        x, y = position

        directions = self.side[orientation]
        ...
        for action, (dx, dy) in directions.items():
            ...
            if 0<=nx<10 and 0<=ny<10 and (nx, ny) not in self.walls:
                ...
                if action == "Turn left" or action == "Turn right" or action == "Move backward":
                    new_orientation = changeOrientation[action]
                new_dots = dots
                if (nx, ny) in dots:
                    new_dots = tuple(sorted(t for t in dots if t != (nx, ny)))
                successors[action] = ((nx, ny), new_orientation, new_dots)
...
if __name__ == "__main__":
    ...
    y = int(input())
    direction = input()
    numberDots = int(input())
    dots = tuple([tuple(map(int, input().split(","))) for _ in range(numberDots)])
    ...
    problem = Zadaca(((x,y), direction, dots), walls)
