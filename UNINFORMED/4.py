...
class Zadaca(Problem):
    def __init__(self, initial, gridSize, opps, goal=None):
    ...
    def successor(self, state):
        ...
        for ball in state:
            ...
            for action, (dx, dy) in self.dirs.items():
                nx = x + dx
                ny = y + dy
                if 0<=nx<self.n and 0<=ny<self.n and (nx, ny) in state:
                    nnx = nx + dx
                    nny = ny + dy
                    if 0<=nnx<self.n and 0<=nny<self.n and (nnx, nny) not in state and (nnx, nny) not in self.opps:
                        newState = list(t for t in state if t!=(nx, ny) and t!=(x, y))
                        newState.append((nnx, nny))
                        newState = tuple(newState)
                        successors[f"{action}: (x={x},y={y})"] = newState
        return successors
...
if __name__ == "__main__":
    ...
    balls = tuple(tuple(map(int, input().split(","))) for _ in range(ballsCount))
    oppsCount = int(input())
    opps = tuple(tuple(map(int, input().split(","))) for _ in range(oppsCount))

    problem = Zadaca(balls, girdSize, opps)
