...
class Zadaca(Problem):
    def __init__(self, initial, n, goalColor):
    ...
    def successor(self, state):
        ...
        for i, tile in enumerate(state):
            ...
            successors[f"x: {y}, y: {x}"] = new_state
        return successors
    ...
    def goal_test(self, state):
        return state == self.goalColor


if __name__ == "__main__":
    n = int(input())
    boardColors = tuple(map(int, input().split(",")))
    goalColor = tuple(1 for _ in range(len(boardColors)))

    problem = Zadaca(boardColors, n, goalColor)
