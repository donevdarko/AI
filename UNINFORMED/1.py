...
class Box(Problem):
    ...
    def goal_test(self, state):
        ...
        return first == second == self.end_point

    @staticmethod
    def check_valid(state):
        manPos, firstPos, secondPos = state
        
        if not (0 <= firstPos[0] < 5 and 0 <= firstPos[1] < 5):
            return False
        if not (0 <= firstPos[0] < 5 and 0 <= firstPos[1] < 5):
            return False

        if firstPos == secondPos and firstPos != (4, 4):
            return False
        return True
    ...
    def successor(self, state):
        ...
        manPos, firstBoxPos, secondBoxPos = state
        x, y = manPos
        fbx, fby = firstBoxPos
        sbx, sby = secondBoxPos
        ...
        for action, (dx, dy) in directions.items():
            ...
            if 0<=nx<5 and 0<=ny<5:
                if (nx, ny) == firstBoxPos and self.check_valid(((nx, ny), (nfbx, nfby), secondBoxPos)):
                    succ[f'Push box 1 {action}'] = ((nx, ny), (nfbx, nfby), secondBoxPos)
                if (nx, ny) == secondBoxPos and self.check_valid(((nx, ny), firstBoxPos, (nsbx, nsby))):
                    succ[f'Push box 2 {action}'] = ((nx, ny), firstBoxPos, (nsbx, nsby))
                if (nx, ny) not in (firstBoxPos, secondBoxPos):
                    succ[f'Move man {action}'] = ((nx, ny), firstBoxPos, secondBoxPos)
