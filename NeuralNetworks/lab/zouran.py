...
if __name__ == '__main__':
    ...
    split = len(dataset) * 80 // 100
    train_full = dataset[:split]
    test = dataset[split:]
    ...
    n = len(train_full) // 3
    folds = [train_full[:n], train_full[n:2*n], train_full[2*n:]]
    ...
    for i in range(3):
        ...
        clf = MLPClassifier(hidden_layer_sizes=(num_neurons,),
                            learning_rate_init=0.001, max_iter=20, random_state=0)
        clf.fit(train_X_s, train_Y)
        regular_acc.append(clf.score(val_X_s, val_Y))

        # Ensemble
        c1 = MLPClassifier(hidden_layer_sizes=(5,),
                           learning_rate_init=0.001, max_iter=20, random_state=0)
        c2 = MLPClassifier(hidden_layer_sizes=(10,),
                           learning_rate_init=0.001, max_iter=40, random_state=0)
        c1.fit(train_X_s, train_Y)
        c2.fit(train_X_s, train_Y)

        p1 = c1.predict_proba(val_X_s)
        p2 = c2.predict_proba(val_X_s)
        classes = c1.classes_  # assumes both have same class order

        hits = 0
        for j, true_y in enumerate(val_Y):
            max1 = p1[j].max()
            max2 = p2[j].max()
            if max1 >= max2:
                pred = classes[p1[j].argmax()]
            else:
                pred = classes[p2[j].argmax()]
            if pred == true_y:
                hits += 1
        ensemble_acc.append(hits / len(val_Y))

    avg_regular = sum(regular_acc) / 3
    avg_ensemble = sum(ensemble_acc) / 3

    # Retrain winner on full training set, then test
    train_X = [row[:-1] for row in train_full]
    train_Y = [row[-1] for row in train_full]
    scaler = MaxAbsScaler()
    train_X_s = scaler.fit_transform(train_X)
    test_X_s = scaler.transform(test_X)

    if avg_regular >= avg_ensemble:
        clf = MLPClassifier(hidden_layer_sizes=(num_neurons,),
                            learning_rate_init=0.001, max_iter=20, random_state=0)
        clf.fit(train_X_s, train_Y)
        test_score = clf.score(test_X_s, test_Y)
    else:
        c1 = MLPClassifier(hidden_layer_sizes=(5,),
                           learning_rate_init=0.001, max_iter=20, random_state=0)
        c2 = MLPClassifier(hidden_layer_sizes=(10,),
        ...
        c1.fit(train_X_s, train_Y)
        c2.fit(train_X_s, train_Y)
        p1 = c1.predict_proba(test_X_s)
        p2 = c2.predict_proba(test_X_s)
        classes = c1.classes_
        hits = 0
        for j, true_y in enumerate(test_Y):
            if p1[j].max() >= p2[j].max():
                pred = classes[p1[j].argmax()]
            else:
                pred = classes[p2[j].argmax()]
            if pred == true_y:
                hits += 1
        test_score = hits / len(test_Y)
